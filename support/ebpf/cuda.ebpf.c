...
// gpu_target_pids filters CUDA USDT sampling by tgid. The "all pids" sentinel
// (key 0) means "sample every process"; when it is absent only the listed
// tgids are sampled.
bpf_map_def SEC("maps") gpu_target_pids = {
...
static inline __attribute__((__always_inline__)) bool gpu_should_trace_pid(u32 pid)
{
  u32 key0 = 0;
  return bpf_map_lookup_elem(&gpu_target_pids, &key0) || bpf_map_lookup_elem(&gpu_target_pids, &pid);
}
